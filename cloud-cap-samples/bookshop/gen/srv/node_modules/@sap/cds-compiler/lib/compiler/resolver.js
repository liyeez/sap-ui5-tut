...
function resolve( model ) {
  ...
  function populateQuery( query ) {
    ...
    function resolveExcluding( name ) {
      ...
      if (options.testMode) {
        compileMessageRef.validNames = query._combined;
        info( null, query.excludingDict[name].location, {},
              `Valid: ${ Object.keys( query._combined ).sort().join(', ') }` );
      }
  ...
  function redirectImplicitly( elem, assoc ) {
    ...
    if (service &&
        service !== (assoc._main || assoc)._service &&
      ...
      const elemScope = scopedRedirections && // null if no scoped redirections
                        preferredElemScope( target, service, elem,
                                            definitionScope( assoc._main || assoc ) );
  ...
  function preferredElemScope( target, service, elem, assocScope ) {
    const targetScope = definitionScope( target );
    if (targetScope === assocScope)
      return definitionScope( elem._main || elem ); // own scope, then global
  ...
  function minimalExposure( target, service, elemScope ) {
    // TODO: prefer name which we would use for autoexposed
    if (!target._descendants)
      return [];
    const descendants = scopedExposure( target._descendants[service.name.absolute] || [],
                                        elemScope, target );
  ...
  function scopedExposure( descendants, elemScope, target ) {
    ...
    if (elemScope.kind === 'entity' && // not event
        projectionAncestor( target, elemScope.params ) && // same params
        // direct proj (TODO: or should we add them to another list?)
        elemScope.query.op && elemScope.query.op.val === 'SELECT' &&
        elemScope._from && elemScope._from.length === 1 &&
        target === resolvePath( elemScope._from[0], 'from', elemScope ))
      return [ elemScope ];
  ...
  function createAutoExposed( target, service, elemScope ) {
    ...
    if (autoexposed && (autoexposed.kind !== 'namespace' || !scopedRedirections)) {
      // TODO: if there is a projection on target on that place, use that - is possible later
  ...
  function initFromColumns( query ) {
    ...
    function expandWildcard( elements, location ) {
      ...
      for (const name in query._combined) {
        const navElem = query._combined[name];
  ...
  function resolveQuery( query ) {
    ...
    if (query.from)
      resolveJoinOn( query.from, 'expr', query, query._combined );
    if (query.where)
  ...
  function resolveExpr( expr, expected, user, extDict) {
    ...
    if (expr.path) {
      ...
      args.forEach( e => resolveExpr( e, expected, user, extDict ) );
    }
  }
  ...
  function resolveParamsAndWhere( step, expected, user, extDict, isLast ) {
    ...
    if (!art)
      return;
    const entity = (art.kind === 'entity') && (!isLast || expected === 'from') && art;
    if (step.args)
  ...
  function resolveParams( dict, art, entity, expected, user, extDict, stepLocation ) {
    ...
    if (Array.isArray(dict)) {
      message( 'args-expected-named', [ dict[0] && dict[0].location || stepLocation, user ],
               'Named parameters must be provided for the entity' ); // TODO: error
      for (const a of dict)
