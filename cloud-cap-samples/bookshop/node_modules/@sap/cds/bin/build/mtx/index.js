...
class MtxModuleBuilder extends BuildTaskHandlerInternal {
     ...
     */
    _validateExtensionWhitelists(model, tenantDbPath) {
        ...
        if (!Array.isArray(entityWhitelist) || !Array.isArray(serviceWhitelist)) {
            this.pushMessage("No 'entity-whitelist' or 'service-whitelist' defined. By default all entities and services can be extended.", WARNING) //NOSONAR
        }

        if (Array.isArray(entityWhitelist) || Array.isArray(serviceWhitelist)) {
            // validate whitelist entries
            const errornousEntries = new Set()
            const csn = this.cds.reflect(model)
            if (Array.isArray(entityWhitelist)) {
                entityWhitelist.forEach(entityName => {
                    if (!csn.find(entity => entity.name === entityName && entity.kind === 'entity')) {
            ...
            if (Array.isArray(serviceWhitelist)) {
                serviceWhitelist.forEach(serviceName => {
                    if (!csn.find(service => service.name === serviceName && service.kind === 'service')) {
