...
class LinkedCSN extends any {
  ...
  constructor(x) {
    ...
    function _link (d, name, parent, _kind) {
      ...
      if (d.kind === 'element' && p.key) Object.defineProperty (d,'key',{value:undefined})  //> don't propagate .key
  ...
  minified() {
    const { skip_unused } = global.cds.env.features; if (!skip_unused) return this
    const csn = this, all = csn.definitions, reached = new Set
    for (let each of skip_unused === 'all' ? this.services : this.each(_root)) _visit (each)
    function _visit (d) {
      ...
      if (d.type)      d.type.ref || _builtin(d.type) || _visit (d.__proto__)
    ...
    for (let d in all) if (!reached.has(all[d])) {
      if (d.endsWith('.texts')) delete all[d.slice(0,-6)+'_texts']
      delete all[d]
    }
    return csn
  }
